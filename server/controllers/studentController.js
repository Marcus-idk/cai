...
async function handleSubmitForm(req, res) {
    try {
        ...
        const { codingLanguages, projectInterests, frameworks, interests } = JSON.parse(req.body.data);
        ...
        let combinedInterests = [interestsString, ...sortedAndFormattedProjectInterests].join(', ');
        console.log('Combined Interests:', combinedInterests);
        
        const languagesAndFrameworksTags = await callPythonScript('processTagsForLangsAndFrameworks', languagesAndFrameworks, '');
        const companyInterestsTags = await callPythonScript('processTagsForCompanies', combinedInterests, '');        

        console.log("Languages and Frameworks Tags:", languagesAndFrameworksTags);
        console.log("Company Interests Tags:", companyInterestsTags);
        tags = languagesAndFrameworksTags.concat(companyInterestsTags)
        console.log("Final combination: ", tags)
